...
# ---------------------------------------------------------------------------
# Custom CSS — tema claro + paleta institucional FVL
# ---------------------------------------------------------------------------
...
_MAX_CONTEXT_CHARS = 150_000  # gpt-4o-mini: 128k tokens. Español ≈ 1.5 chars/token → 150k ≈ 100k tokens.
...
with st.sidebar:
    ...
    st.metric("Caracteres indexados", f"{stats['estimated_chars']:,}")
...
st.markdown(
    ...
    <div class="main-header">
        ...
        <p>Base de conocimiento semántico extraída del sitio oficial · {stats['total_documents']} documentos · {len(stats['categories'])} categorías</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with tab_qa:
    ...
    # ── Registro de pruebas para las 20 preguntas del informe ──
    if st.session_state["qa_history"]:
        ...
        with col_a:
            if st.button("Agregar última a resultados", use_container_width=True):
                last = st.session_state["qa_history"][-1]
                st.session_state["qa_log"].append(
                    {"question": last["question"], "answer": last["answer"]}
                )
        with col_b:
            if st.button("Limpiar registro CSV", use_container_width=True):
                st.session_state["qa_log"] = []
        ...
        with col_c:
            st.download_button(
                label=f"⬇️ Descargar CSV ({len(st.session_state['qa_log'])})",
                data=csv_buffer.getvalue(),
                file_name="qa_results.csv",
                mime="text/csv",
                use_container_width=True,
            )
        st.caption("Guardar como `tests/qa_results.csv` para el informe.")
